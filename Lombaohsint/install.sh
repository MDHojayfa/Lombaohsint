...
case $OS in
    ...
    KALI|UBUNTU|DEBIAN)
        ...
        # --- CRITICAL: VERIFY venv IS AVAILABLE ---
        if ! python3 -m venv --help >/dev/null 2>&1; then
            error "Python3 venv module is missing. Install python3-venv manually: apt install python3-venv"
        fi
        ...
        source "$BASE_DIR/venv/bin/activate" || error "Failed to activate virtual environment. File missing: $BASE_DIR/venv/bin/activate"
